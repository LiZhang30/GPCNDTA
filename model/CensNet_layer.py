...
class GraphConvolution(Module):
    ...
    def forward(self, H_v, H_e, adj_e, adj_v, T):
        if self.node_layer:
            ...
            mask1 = torch.eye(multiplier1.shape[0]).to('cuda:0')
            M1 = mask1 * torch.ones(multiplier1.shape[0]).to('cuda:0') + (1. - mask1)*multiplier1
            ...
            mask2 = torch.eye(multiplier2.shape[0]).to('cuda:0')
            M3 = mask2 * torch.ones(multiplier2.shape[0]).to('cuda:0') + (1. - mask2)*multiplier2
